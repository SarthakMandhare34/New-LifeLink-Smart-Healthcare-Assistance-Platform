  ...
  if (isLoading) {
    return (
      <div className="dashboard-loading" style={{ padding: '32px' }}>
        <p className="caption" style={{ color: 'var(--color-doctor-primary)' }}>Loading prescriptions workspace…</p>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '36px', width: '100%' }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', gap: '18px' }}>
        ...
        <div>
          ...
          <p style={{ color: 'var(--color-text-muted)', fontSize: '0.92rem', margin: '4px 0 0' }}>
      ...
      {/* =====================================================================================
          ...
          ===================================================================================== */}
      <section style={{ display: 'flex', flexDirection: 'column', gap: '20px' }} aria-labelledby="issued-prescriptions-heading">
        ...
        {issuedPrescriptions.length === 0 ? (
          <Card style={{ ...cardStyle, textAlign: 'center', padding: '48px 24px' }}>
            <FileText size={44} color="var(--color-text-muted)" style={{ margin: '0 auto 14px', opacity: 0.6 }} />
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 320px), 1fr))', gap: '24px' }}>
            {issuedPrescriptions.map((rx) => (
              <Card key={rx.id} style={cardStyle}>
                ...
                {rx.clinicalNotes && (
                  <p style={{ margin: '0 0 6px', fontSize: '0.84rem', color: 'var(--color-text)', fontStyle: 'italic' }}>
                ...
                {rx.integrityReference && (
                  <div style={{ marginTop: 'auto', paddingTop: '12px', borderTop: '1px solid var(--color-border)' }}>
                    ...
                    <p style={{ margin: '4px 0 0', fontSize: '0.7rem', fontFamily: 'monospace', color: 'var(--color-text)', wordBreak: 'break-all', overflowWrap: 'anywhere' }}>
                      {rx.integrityReference}
                    </p>
                  </div>
                )}
      ...
      {/* =====================================================================================
          ...
          ===================================================================================== */}
      <section style={{ display: 'flex', flexDirection: 'column', gap: '20px' }} aria-labelledby="eligible-patients-heading">
        <div>
        ...
        {assignedPatients.length === 0 ? (
          <Card style={{ ...cardStyle, textAlign: 'center', padding: '48px 24px' }}>
            <FileText size={44} color="var(--color-text-muted)" style={{ margin: '0 auto 14px', opacity: 0.6 }} />
            <h3 style={{ margin: '0 0 6px', color: 'var(--color-doctor-text)', fontSize: '1.05rem' }}>No eligible patient appointments</h3>
            <p style={{ margin: '0 0 18px', color: 'var(--color-text-muted)', fontSize: '0.88rem' }}>
